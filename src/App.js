...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (showSplash) {
      interval = setInterval(() => {
        setSplashDotCount((prev) => (prev + 1) % 4);
      }, 400);
    }
  ...
  useEffect(() => {
    ...
    const trackKey =
      targetKey === 'night_duty' || targetKey === 'night_choice'
        ? 'night'
        : targetKey;
  ...
  const onTouchEnd = () => {
    if (touchStart.current.x === null || touchEnd.current.x === null) {
      touchStart.current = { x: null, y: null, time: null };
      return;
    }
  ...
  const saveToHistory = () => {
    setScheduleHistory((prev) =>
      [
        ...prev,
        JSON.parse(JSON.stringify({ schedules: masterData.schedules || {} })),
      ].slice(-10)
    );
    setRedoHistory([]);
  };
  ...
  const handleUndo = () => {
    ...
    setMasterData((prev) => ({ ...prev, schedules: previousState.schedules }));
  };
  ...
  const handleRedo = () => {
    ...
    setMasterData((prev) => ({ ...prev, schedules: nextState.schedules }));
  };
  ...
  const applyPattern = (group) => {
    ...
    if (selectedDayKey && drawMode === 'shift') {
      ...
      endDate = new Date(y + 5, 11, 31, 0, 0, 0);
    } else {
      startDate = new Date(2026, 0, 1, 0, 0, 0);
      endDate = new Date(2030, 11, 31, 0, 0, 0);
    }
    const totalDaysToApply =
  ...
  const renderDayCell = (d, monthType) => {
    ...
    const month =
      ...
      (monthType === 'prev' ? -1 : monthType === 'next' ? 1 : 0);
    ...
    const holName = cachedHolidays[key];
    const isToday = new Date().toDateString() === dateObj.toDateString();
    const isSelected = selectedDayKey === key;

    const dateColor =
      dateObj.getDay() === 0 || holName
        ? 'text-[#e55a5a]'
        : dateObj.getDay() === 6
        ? 'text-[#5a8be5]'
        : 'text-slate-400';
    const bgClass = isSelected
      ? 'bg-[#2a2a2c]/80 border border-[#60a5fa]/50 scale-[1.02]'
      : 'bg-[#1c1c1e]/60 border border-white/5';
    ...
    return (
      <div
        ...
        onClick={() => {
          if (isEditing) {
            if (showShiftMenu || showNightMenu) {
              setShowShiftMenu(false);
            ...
            if (drawMode === 'erase') {
              ...
              if (newData.schedules[key]?.memo)
                delete newData.schedules[key].memo;
              setMasterData(newData);
            } else if (
              [
                'day',
              ...
              saveToHistory();
              const newData = JSON.parse(JSON.stringify(masterData));
              if (!newData.schedules[key]) newData.schedules[key] = {};
              newData.schedules[key].type = drawMode;
              if (drawMode === 'schedule') {
                newData.schedules[key].text = loadedScheduleText;
              }
              setMasterData(newData);
            }
          } else {
            if (s.memo)
              setViewModal({ isOpen: true, memo: s.memo, dateKey: key });
          }
        }}
        className={`flex-1 h-[105px] mx-[2px] rounded-[1.1rem] relative flex flex-col shadow-sm cursor-pointer overflow-hidden ${bgClass} ${
          monthType !== 'current' ? 'opacity-40' : ''
        } active:scale-[0.92] transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] select-none`}
        style={{
      ...
      >
        <div
          className="w-full flex justify-between items-start px-2 pt-1.5 shrink-0 h-[22px]"
          style={{ pointerEvents: 'none' }}
        >
          <span className={`text-[11px] font-black ${dateColor}`}>{d}</span>
          {holName && (
            <div className="flex flex-col items-end leading-[9px] text-right">
              {formatHolidayText(holName).map((line, idx) => (
                <span
                  key={idx}
                  className="text-[7px] font-black text-[#e55a5a] whitespace-nowrap"
                >
                  {line}
                </span>
              ))}
        ...
        <div
          className="w-full h-[22px] flex justify-center items-center shrink-0"
          style={{ pointerEvents: 'none' }}
        >
          {s.type && (
            <div className={`${maps[s.type]?.tc} drop-shadow-sm scale-[0.85]`}>
              {maps[s.type]?.i}
            </div>
          )}
        </div>
        ...
        >
          ...
          <div className="w-full h-[18px] flex items-start justify-center overflow-hidden shrink-0">
            ...
            {s.type === 'night_duty' && (
              <div className="w-[92%] mx-auto text-[8px] font-black py-[2px] rounded-full text-center bg-[#2e1065]/40 border-[0.5px] border-[#4c1d95]/70 text-[#c4b5fd] truncate">
                당직
              </div>
            )}
          </div>
  ...
  const renderDaysGrid = () => {
    ...
    for (let i = firstDay - 1; i >= 0; i--)
      days.push(renderDayCell(prevMonthLastDate - i, 'prev'));
    for (let d = 1; d <= lastDate; d++) days.push(renderDayCell(d, 'current'));
    const rows = Math.ceil(days.length / 7);
    const targetCells = rows * 7;
    const remaining = targetCells - days.length;
    for (let i = 1; i <= remaining; i++) days.push(renderDayCell(i, 'next'));
    const gridRows = [];
    for (let i = 0; i < days.length; i += 7)
      gridRows.push(
        <div key={i} className="flex justify-between mb-2 px-1 flex-1">
          {days.slice(i, i + 7)}
        </div>
      );
    return gridRows;
  ...
  return (
    <>
      ...
      {toast && (
        <div
          className="fixed top-12 left-1/2 -translate-x-1/2 z-[99999] animate-in slide-in-from-top-2 fade-in duration-200"
          style={{ pointerEvents: 'none' }}
        >
          <div className="bg-[#1c1c1e]/70 backdrop-blur-xl frost-border px-4 py-2.5 rounded-full flex items-center justify-center min-w-[140px]">
      ...
      {showSplash && (
        <div
          className={`fixed inset-0 z-[99999] flex flex-col items-center justify-center transition-opacity duration-500 ${
            isAuthReady && isMinLoadingTimeDone ? 'opacity-0' : 'opacity-100'
          }`}
          style={{
            background: 'linear-gradient(135deg, #1a050a 0%, #4a161c 100%)',
            pointerEvents: 'none',
          }}
        >
          <div className="relative z-10 flex flex-col items-center w-full px-6">
            ...
            <div className="relative z-10 flex justify-center text-[12px] font-black tracking-widest text-[#D4AF37]/70 drop-shadow-md animate-pulse">
              <span className="relative inline-block">
                CONNECTING
                <span className="absolute left-full top-0 w-[24px] text-left">
                  {'.'.repeat(splashDotCount)}
                </span>
              </span>
            </div>
      ...
      <div
        className="max-w-[500px] mx-auto min-h-screen flex flex-col relative shadow-2xl font-sans overflow-hidden"
        style={{ backgroundColor: '#0a0c10' }}
      >
        <header
          className="pt-3 pb-3 px-5 rounded-b-[2rem] shadow-xl z-20 relative border-b border-[#D4AF37]/20"
          style={{
            background: 'linear-gradient(145deg, #4a161c 0%, #1a050a 100%)',
          }}
        >
          ...
          <div className="flex justify-between items-center mb-1 px-2 relative z-30">
            <button
              ...
              className="p-1 relative z-40 transition-transform active:scale-[0.85]"
              style={{ WebkitTapHighlightColor: 'transparent' }}
            >
            ...
            <div className="flex flex-col items-center justify-center">
              <h1
                ...
                className="text-2xl font-black tracking-tighter text-white drop-shadow-sm cursor-pointer px-4 py-2 relative z-40 select-none transition-transform active:scale-[0.92]"
                style={{ WebkitTapHighlightColor: 'transparent' }}
              >
        ...
        <div
          className={`overflow-hidden transition-all duration-500 ease-in-out z-20 ${
            showSettings
              ? 'max-h-[800px] opacity-100 mt-2 px-4'
              : 'max-h-0 opacity-0'
          }`}
        >
              ...
              <div className="relative w-[65%] h-12">
                ...
                {isEditing && (
                  <button
                    ...
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-white/5 hover:bg-white/10 rounded-full transition-transform active:scale-[0.85]"
                    style={{ WebkitTapHighlightColor: 'transparent' }}
                  >
            ...
            {isEditing && showPwdChange && (
              <div className="flex items-center justify-between gap-2.5 pt-2 mt-1 animate-in slide-in-from-top-2 fade-in duration-200">
                <input
                  ...
                  className="w-[65%] h-12 px-5 bg-black/40 border border-[#60a5fa]/40 rounded-full text-[16px] font-bold text-white outline-none focus:border-[#60a5fa]/80 transition-colors"
                />
                <button
                  onClick={async () => {
                    if (!newPass)
                      return showToast('🟡 새 비밀번호를 입력하세요');
                    const newData = { ...masterData, editPassword: newPass };
                    setMasterData(newData);
                    if (familyKey) {
                      const docRef = doc(
                        db,
                  ...
                  className="w-[32%] h-12 rounded-full font-black text-[12px] border border-[#60a5fa]/50 text-[#60a5fa] bg-[#60a5fa]/10 transition-transform active:scale-[0.92]"
                  style={{ WebkitTapHighlightColor: 'transparent' }}
                >
                  변경저장
                </button>
        ...
        >
          <div className="flex justify-between items-center px-5 py-3 mx-1 mb-4 bg-white/5 backdrop-blur-md frost-border rounded-full relative z-10">
            <span className="text-[12px] font-black text-white/60 tracking-widest">
              {currentDate.getMonth() + 1}월 근무
            </span>
            <div className="flex gap-3">
              <div className="flex items-center gap-1">
                <Sun size={14} className="text-yellow-500" />
                <span className="text-[13px] font-black text-white">
                  {monthlyWorkCount.day}
                </span>
              </div>
              <div className="flex items-center gap-1">
                <Moon size={14} className="text-indigo-400" />
                <span className="text-[13px] font-black text-white">
                  {monthlyWorkCount.night}
                </span>
              </div>
              <div className="flex items-center gap-1 pl-1 border-l border-white/10">
                <Bell size={14} className="text-violet-400" />
                <span className="text-[13px] font-black text-white">
                  {monthlyWorkCount.duty}
                </span>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-7 text-center py-2 text-[10px] font-black text-slate-500 mb-1 uppercase tracking-[0.2em] border-b border-white/5 relative z-10">
            {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((w, i) => (
              <span
                key={w}
                className={
                  i === 0 ? 'text-[#e55a5a]' : i === 6 ? 'text-[#5a8be5]' : ''
                }
              >
                {w}
              </span>
                  ...
                  <button
                    ...
                    className={`w-[36px] h-[36px] flex items-center justify-center rounded-full ${
                      scheduleHistory.length > 0
                        ? 'text-white active:scale-75 transition-transform'
                ...
                <div className="pointer-events-auto animate-modal-spring flex justify-center w-max">
                  {showShiftMenu && (
                    <div className="bg-[#1c1c1e]/95 backdrop-blur-3xl frost-border p-1.5 rounded-[1.8rem] flex gap-2 shadow-[0_15px_45px_rgba(0,0,0,0.8)] border border-white/15">
                      <button
                        onClick={() => applyPattern('gap')}
                        className="px-5 py-2.5 bg-blue-500/20 border border-blue-500/30 rounded-[1.3rem] text-[13px] font-black text-blue-300 transition-transform active:scale-[0.85] select-none whitespace-nowrap"
                      >
                        갑조
                      </button>
                      <button
                        onClick={() => applyPattern('eul')}
                        className="px-5 py-2.5 bg-emerald-500/20 border border-emerald-500/30 rounded-[1.3rem] text-[13px] font-black text-emerald-300 transition-transform active:scale-[0.85] select-none whitespace-nowrap"
                      >
                        을조
                      </button>
                      <button
                        onClick={() => applyPattern('byeong')}
                        className="px-5 py-2.5 bg-purple-500/20 border border-purple-500/30 rounded-[1.3rem] text-[13px] font-black text-purple-300 transition-transform active:scale-[0.85] select-none whitespace-nowrap"
                      >
                        병조
                      </button>
                    </div>
                  ...
                  {showNightMenu && (
                    <div className="bg-[#1c1c1e]/95 backdrop-blur-3xl frost-border p-1.5 rounded-[1.8rem] flex gap-2 shadow-[0_15px_45px_rgba(0,0,0,0.8)] border border-white/15">
                      <button
                        ...
                        className="px-7 py-2.5 bg-indigo-500/20 border border-indigo-500/30 rounded-[1.3rem] text-[13px] font-black text-indigo-300 transition-transform active:scale-[0.85] select-none whitespace-nowrap"
                      >
                        비당직
                      </button>
                      <button
                        onClick={() => {
                          setDrawMode('night_duty');
                          setShowNightMenu(false);
                        }}
                        className="px-7 py-2.5 bg-violet-500/20 border border-violet-500/30 rounded-[1.3rem] text-[13px] font-black text-violet-300 transition-transform active:scale-[0.85] select-none whitespace-nowrap"
                      >
                        당직
                      </button>
                    </div>
            ...
            <div className="fixed bottom-2 left-1/2 -translate-x-1/2 w-full max-w-[500px] px-4 z-[9999] pointer-events-none">
              <div className="w-full flex items-end gap-3 pointer-events-none relative">
                <div className="flex-1 relative flex flex-col min-w-0 pointer-events-auto">
                  <div className="w-full h-[64px] bg-[#1a1c23]/70 backdrop-blur-xl frost-border rounded-[2.5rem] flex items-center relative overflow-hidden shadow-[0_10px_20px_rgba(0,0,0,0.5)]">
                    ...
                    >
                      <div
                        ...
                        style={{
                          ...
                          left: indicatorStyle.left
                            ? indicatorStyle.left - 6
                            : 0,
                          width: indicatorStyle.width
                            ? indicatorStyle.width + 12
                            : 0,
                          backgroundColor: 'rgba(255, 255, 255, 0.1)',
                          opacity: indicatorStyle.opacity,
                          transition: indicatorStyle.transition,
                          transform: `scale(${indicatorStyle.scale || 1})`,
                          transformOrigin: 'center',
                        }}
                      />
                      {Object.entries(maps)
                        .filter(([id]) => id !== 'night_duty')
                        .map(([id, t]) => {
                          const isSelected =
                            (drawMode === id ||
                              (id === 'night' &&
                          ...
                          return (
                            <button
                              ...
                              className="min-w-[48px] h-[52px] flex flex-col items-center justify-center gap-1.5 relative group z-10 origin-center select-none shrink-0"
                              style={{ WebkitTapHighlightColor: 'transparent' }}
                            >
                              <div
                              ...
                              <span
                                className={`text-[8px] uppercase tracking-wider transition-all duration-300 z-10 ${
                                  isSelected
                                    ? `${t.tc} font-black scale-105`
                      ...
                      <button
                        key="shift"
                        ref={(el) => (buttonRefs.current['shift'] = el)}
                        onPointerDown={(e) => {
                        ...
                        className="min-w-[48px] h-[52px] flex flex-col items-center justify-center gap-1.5 relative group z-10 origin-center select-none shrink-0 mr-1"
                        style={{ WebkitTapHighlightColor: 'transparent' }}
                      >
                        <div
                          className={`transition-all duration-300 z-10 ${
                            showShiftMenu || drawMode === 'shift'
                              ? `scale-[1.1] text-[#60a5fa]`
                              : 'text-white/40'
                          }`}
                        >
                          <RefreshCw size={20} />
                        ...
                        <span
                          className={`text-[8px] uppercase tracking-wider transition-all duration-300 z-10 ${
                            showShiftMenu || drawMode === 'shift'
                              ? 'text-[#60a5fa] font-black scale-105'
                              : 'text-white/30 font-bold'
                          }`}
                        >
                          SHIFT
                ...
                <button
                  ...
                  className="w-[64px] h-[64px] shrink-0 rounded-full bg-gradient-to-br from-[#4a161c] to-[#2a0b10] border border-[#D4AF37]/40 shadow-[0_0_20px_rgba(212,175,55,0.25)] flex flex-col items-center justify-center gap-1 pointer-events-auto transition-transform active:scale-[0.85] select-none"
                  style={{ WebkitTapHighlightColor: 'transparent' }}
                >
                  <Save size={20} className="text-[#D4AF37]" />
                  <span className="text-[10px] font-black tracking-wider text-[#D4AF37]">
                    SAVE
                  </span>
                </button>
              </div>
        ...
        {modalState.isOpen && (
          <div
            ...
            style={{ WebkitTapHighlightColor: 'transparent' }}
          >
            {modalState.type === 'scheduleInput' && (
              <div className="bg-[#1e293b]/70 backdrop-blur-2xl frost-border w-full max-w-[260px] rounded-[2rem] p-5 animate-modal-spring shadow-2xl shadow-black/60">
                <h3 className="text-[17px] font-black mb-4 text-white text-center">
                  일정 입력
                </h3>
                <input
                  type="text"
                  maxLength={2}
                  placeholder="예: 연차, 휴가"
                  className="w-full bg-[#0f172a]/80 rounded-xl p-3 text-[16px] font-bold text-center text-white outline-none border border-white/10 focus:border-[#60a5fa]/50 transition-colors"
                ...
                <div className="flex gap-2 mt-4">
                  <button
                    ...
                    className="flex-1 py-2.5 border border-white/20 text-white/60 rounded-[1rem] font-black text-[14px] transition-transform active:scale-[0.92]"
                  >
                    취소
                  </button>
                  <button
                    onClick={() => {
                      setLoadedScheduleText(modalState.data.text || '일정');
                      setDrawMode('schedule');
                      setModalState({ isOpen: false });
                    }}
                    className="flex-1 py-2.5 border border-cyan-500/60 text-cyan-400 bg-cyan-500/10 rounded-[1rem] font-black text-[14px] transition-transform active:scale-[0.92]"
                  >
                    확인
                  </button>
                </div>
            ...
            {modalState.type === 'memo' && (
              <div className="bg-[#131b2c]/70 backdrop-blur-2xl frost-border w-full max-w-[280px] rounded-[2rem] p-5 animate-modal-spring shadow-2xl shadow-black/60">
                <div className="flex justify-between items-center mb-4">
                  <span className="text-white font-black text-[17px]">
                    {formatPopupDate(modalState.dateKey)}
                  </span>
                  <button
                    onClick={() => setModalState({ isOpen: false })}
                    className="text-slate-400 p-1 bg-white/5 rounded-full transition-transform active:scale-[0.85]"
                  >
                    <X size={18} />
                  </button>
                </div>
                <textarea
                  className="w-full h-28 bg-[#1e293b]/80 rounded-[1.2rem] p-3 text-[16px] font-bold text-white outline-none resize-none border border-white/10 focus:border-[#60a5fa]/50 transition-colors"
                  value={modalState.data.text || ''}
                  onChange={(e) =>
                    setModalState((prev) => ({
                      ...prev,
                ...
                <button
                  onClick={() => {
                    saveToHistory();
                    const newData = JSON.parse(JSON.stringify(masterData));
                    if (!newData.schedules[modalState.dateKey])
                      newData.schedules[modalState.dateKey] = {};
                    newData.schedules[modalState.dateKey].memo =
                      modalState.data.text;
                    setMasterData(newData);
                    setModalState({ isOpen: false });
                    showToast('🟢 일정 반영 완료');
                  }}
                  className="w-full py-3 mt-4 rounded-[1rem] font-black border border-blue-500/80 text-blue-400 bg-blue-500/10 text-[14px] transition-transform active:scale-[0.95]"
                >
                  일정반영
                </button>
              </div>
        ...
        {!isEditing && viewModal.isOpen && (
          <div
            ...
            onClick={() =>
              setViewModal({ isOpen: false, memo: null, dateKey: null })
            }
            style={{ WebkitTapHighlightColor: 'transparent' }}
          >
            <div
              className="w-full max-w-[320px] rounded-[2.5rem] p-6 animate-modal-spring shadow-2xl shadow-black/60 bg-[#131b2c]/70 backdrop-blur-2xl frost-border"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex justify-between items-center mb-6">
                <span className="text-white font-black text-xl tracking-wide">
                  {formatPopupDate(viewModal.dateKey)}
                </span>
                <button
                  onClick={() =>
                    setViewModal({ isOpen: false, memo: null, dateKey: null })
                  }
                  className="text-slate-400 p-1.5 bg-white/5 rounded-full transition-colors"
                >
                  <X size={20} />
                </button>
              </div>
              <div className="w-full rounded-[1.25rem] bg-[#1e293b]/50 backdrop-blur-md shadow-inner flex flex-col p-5 text-[15px] font-bold text-white/90 leading-relaxed overflow-y-auto max-h-[30vh] border border-white/10">
                {viewModal.memo && (
